...
const TOP_PAL: Record<string, string[]> = {
  ...
  "1-hoodie": ["#1a1a1e", "#ff8fab", "#7c3aed", "#f4f4f6", "#3b82f6"],
...
function isHairPx(r: number, g: number, b: number, x: number, y: number, w: number) {
  ...
  if (isSide(x, w) && y <= 132 && (brown || (dark && y <= 96))) return true;
  return false;
}
  ...
  for (let y = 76; y < p.h; y++) {
    for (let x = 0; x < p.w; x++) {
      ...
      const r = p.d[i],
        g = p.d[i + 1],
        b = p.d[i + 2];
      if (isPupil(r, g, b, x, y)) continue;
      if (isSkinPx(r, g, b) && y < 96) continue;
    ...
    for (let x = 0; x < dst.w; x++) {
      ...
      if (dst.a(x, y) >= 16) {
        ...
        const dr = dst.d[di],
          dg = dst.d[di + 1],
          db = dst.d[di + 2];
        if (isPupil(dr, dg, db, x, y)) continue;
        if (isSkinPx(dr, dg, db) && y < 96) continue;
...
function tintPixels(
  ...
  for (let y = 0; y < p.h; y++) {
    for (let x = 0; x < p.w; x++) {
      ...
      const r = p.d[i],
        g = p.d[i + 1],
        b = p.d[i + 2];
      if (outside && isInk(r, g, b) && touchesOutside(outside, p.w, p.h, x, y)) continue;
  ...
  const lo = 0.5;
  const hi = Math.min(1, lo + contrast);
  ...
  for (let y = 0; y < p.h; y++) {
    for (let x = 0; x < p.w; x++) {
      ...
      const r = p.d[i],
        ...
        b = p.d[i + 2];
      const idx = y * p.w + x;
      if (isSkinPx(r, g, b)) skinM[idx] = 1;
      else if (isHairPx(r, g, b, x, y, p.w)) hairM[idx] = 1;
    }
  }
  const pal = palOf(f);
  if (f.skin !== 1) {
    tintPixels(p, pal.skin, (x, y) => !!skinM[y * p.w + x], 0.36, outside);
  }
  if (f.hairColor !== 0) {
    tintPixels(p, pal.hair, (x, y) => !!hairM[y * p.w + x], 0.42, outside);
  }
  if (f.top !== 0) {
    tintPixels(
      p,
      pal.top,
      (x, y) => {
        if (y < 78 || y >= BOT_Y + 2 || p.a(x, y) < 16) return false;
        if (isHandZone(x, y, p.w) && y > 132) return false;
        const idx = y * p.w + x;
